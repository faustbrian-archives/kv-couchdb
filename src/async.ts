...
export class StoreAsync<K, T> implements IKeyValueStoreAsync<K, T> {
	...
	public constructor(private readonly opts: { connection: string; database: string }) {
		this.store = nano(opts.connection).db;
	}
	...
	public async flush(): Promise<boolean> {
		try {
			await this.store.create(this.opts.database);
			// tslint:disable-next-line: no-empty
		} catch (error) {}
